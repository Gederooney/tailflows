...
const CookieBanner = () => {
  ...
  const getDisplay = () => {
    const display = localStorage.getItem('display-cookie')
    return display ? display === 'true' : true
  }

  const saveDisplay = () => {
    localStorage.setItem('display-cookie', false.toString())
    setDisplay(false)
  }
  useEffect(() => {
  ...
  return (
    <section className={`${display ? '' : 'sr-only'}`}>
      <div className="fixed bottom-0 end-0 z-[60] sm:max-w-xl w-full mx-auto p-6">
        <div className="p-4 bg-white border border-gray-200 shadow-sm rounded-xl dark:bg-secondary-800 dark:border-gray-50/10">
          <div className="flex gap-x-5">
            <svg
              className="flex-shrink-0 hidden w-20 sm:block"
              width="72"
              height="63"
              viewBox="0 0 72 63"
              fill="none"
            ...
            <div className="grow">
              ...
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                En cliquant "J'ai compris" et en continuant votre navigation sur le site, vous
                acceptez l'utilisation des cookies sur ce site.
              </p>
              <div className="inline-flex mt-5 gap-x-2">
                <button
                  type="button"
                  onClick={(e) => {
                    e.preventDefault()
                  ...
                  className="inline-flex items-center px-3 py-2 text-sm font-semibold text-white border border-transparent rounded-lg bg-primary-600 gap-x-2 hover:bg-primary-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
                >
                  J'ai compris
                </button>
                {/* <button
