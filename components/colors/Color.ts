...
export class Color {
  ...
  static coordinatesToHslV2(
    picker: TCoordinates,
    lightness: TCoordinates
  ): [number, number, number] {
    const hue = this.getHue(picker)
    const saturation = this.getSaturation(picker)
    return [hue, saturation, ((1 - lightness.y) / 2) * 100]
  }

  static hslTolightnessCoordinates([h, s, l]: [number, number, number]) {
    const y = 1 - (l / 100) * 2
  ...
  static hslToRgbV2([h, s, l]: [number, number, number]) {
    s /= 100
    l /= 100
  ...
  static rgbToHexV2({ r, g, b }: { r: number; g: number; b: number }) {
